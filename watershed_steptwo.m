function [IM , new_coor , coor_image , continue_cluster_flag]=watershed_steptwo(bgm,image_type)
%image_type是'binary'或者'rgb'
coor = line_scan(bgm,3,7);%第一个数字是下阈值，第二个是上阈值
[new_coor,coor_image,continue_cluster_flag] = coor_cluster(coor,7);%数字代表聚类的距离阈值
if continue_cluster_flag~=0
  se = strel(ones(3,3));
