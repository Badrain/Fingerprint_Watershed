function [IM,new_coor,coor_image]=red_center(bgm,image_type,upthrehold,downthrehold)
%image_type是'binary'或者'rgb'
coor = line_scan(bgm,upthrehold,downthrehold);%第一个数字是下阈值，第二个是上阈值
%figure;imshow(coor);
[new_coor,coor_image] = coor_cluster(coor,7);%数字代表聚类的距离阈值
%figure;imshow(coor_image);
se = strel(ones(3,3));
